...
fn main() {
    ...
    let shape = a.get(3).map(|s| QrModuleShape::parse(s).expect("shape")).unwrap_or(QrModuleShape::Square);
    let ecc = a.get(4).and_then(|s| QrEcc::parse(s)).unwrap_or(QrEcc::Medium);

    let opts = QrOptions { data, ecc, shape, radius: 0.45, size: 300.0, quiet_zone: 4 };
    ...
    println!("wrote {out}  ({}×{} modules, {:.2} u/module)", art.matrix_size, art.matrix_size, art.module_size);
}
