...
pub fn tool_list() -> Value {
    json!([
        ...
        {
            "name": "list_audit_log",
            "description": "Return the most recent MCP tool calls recorded since the server started.\n\nEach entry includes: `id` (sequential), `timestamp` (ISO 8601), `tool_name`, `args` (a bounded argument summary), `result_summary` (first 200 chars of result text), `duration_ms`, and `is_error`. Large strings, collections, and deeply nested values are truncated before storage.\n\nUseful for multi-agent accountability: see what was called, by whom (if the calling agent passes an `author` in its args), and with which bounded parameters.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "limit": {
        ...
        },
        {
            ...
            "description": "Export the complete in-memory MCP audit log as a JSON array (oldest first). Includes every retained tool call recorded since the server started, up to 1000 entries and the audit byte budget. Argument values are bounded summaries.",
            "inputSchema": {
                "type": "object",
                "properties": {}
            }
