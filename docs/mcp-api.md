...
Combine two or more existing path nodes into a live (non-destructive) compound shape via a boolean op (union/intersect/subtract/exclude). The operand geometry is captured into the compound and the originals are removed; the compound's path is the baked boolean result and stays re-editable (expand it with expand_compound).

| Parameter | Type | Required | Description |
| --- | --- | --- | --- |
| `node_ids` | array<string> | yes | Path node IDs to combine (≥2). The first is the base. |
| `layer_id` | string | no |  |
| `name` | string | no |  |
| `op` | enum (`union`, `intersect`, `subtract`, `exclude`) | no | Boolean mode (default: union). |
...
Expand a live compound shape into a plain path: drops the operands but keeps the baked geometry. No-op for a non-compound node.
