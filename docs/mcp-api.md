...
Save a named snapshot of the current document state and return its checkpoint ID.
...
List all saved document checkpoints, including their IDs, names, and creation timestamps.
...
Restore the document to a saved checkpoint by ID and clear undo/redo history.
