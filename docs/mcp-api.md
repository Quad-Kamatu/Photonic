...
Export the complete in-memory MCP audit log as a JSON array (oldest first). Includes every retained tool call recorded since the server started, up to 1000 entries and the audit byte budget. Argument values are bounded summaries.
