...
> - **§6's rollback runbook — RESOLVED (2026-07-21).** It used to open with "flip the `video` cargo feature off", and no such feature exists — the workspace now ships **zero** cargo features (`photonic-core`'s former `video-p1-contract` gate graduated to always-on, commit `1ccbeea`), and `photonic-gui` depends on `photonic-video` unconditionally. Per the QA-1 decision the kill-switch was **not** implemented (it would buy nothing the revert-range step doesn't) and §6 was rewritten around the revert-range step alone. See §6's runbook bullet.
...
- **Rollback runbook:** every phase merge is revertable as one commit range. Incident procedure: **revert range on a fix branch → re-land** once fixed. Recorded here so mid-incident nobody designs the procedure from scratch. *(Note: there is no `video` cargo kill-switch. An earlier draft opened this runbook with "flip the `video` cargo feature off", but no such feature was ever built — the workspace ships **zero** cargo features and `photonic-gui` depends on `photonic-video` unconditionally (`crates/photonic-gui/Cargo.toml`). Per 11 §7 the feature was intended default-on from P3 and the repo does not carry long-lived feature-flagged forks, so a kill-switch would only add a `video`/`no-video` CI matrix plus `#[cfg(feature)]` at every call site with no incident-time benefit the revert-range step doesn't already give. The revert range is the kill-switch.)*
