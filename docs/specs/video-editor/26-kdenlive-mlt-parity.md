...
| K-0.7 | Sequence-audio muxing on export + `LoudnessTarget` application | ✅ done — offline mix + mux + two-pass loudness gain | [K-D4](#k-d4--per-track-audio-export), [K-F](#14-k-f--render-and-export) | P4/P8 |
| K-0.8 | Wire `EngineCmd::Probe` (or delete the variant and make the direct call the contract) | ✅ done — probe → `set_asset_meta` + invalidate decode | import readiness ladder, [24](24-preview-media-load.md) | P4 |
