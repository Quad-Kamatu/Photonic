...
| K-0.2 | Render the six declared-but-passthrough effects | 🟡 `fca5eda` — params real+hashed; 3/6 kernels (Luma/Chroma/Mask); Blur/Sharpen/Glow remain | all of [K-B](#10-k-b--effects-and-compositing) | P5/P7, [02 §2](02-engine.md) |
...
| K-0.4 | Real directional `Wipe`/`Push` passes | ✅ `3bdf9f6` — `WipeMix`/`PushMix` IR + CPU + WGSL + parity | [K-B7](#k-b7--luma-map-wipes) | P6 |
| K-0.5 | Thread a `lut_provider` into `graph::compile` so `Lut3d` resolves | ✅ `3bdf9f6` — `LutProvider` + session `LutCache` | [07](07-color-grading.md) acceptance | P7 |
...
| K-0.7 | Sequence-audio muxing on export + `LoudnessTarget` application | ⬜ not started | [K-D4](#k-d4--per-track-audio-export), [K-F](#14-k-f--render-and-export) | P4/P8 |
| K-0.8 | Wire `EngineCmd::Probe` (or delete the variant and make the direct call the contract) | ⬜ not started | import readiness ladder, [24](24-preview-media-load.md) | P4 |
| K-0.9 | Make `sync_lock` propagate ripple/insert across sync-locked tracks, **or** remove the control | ✅ done — core expand on insert/extract/ripple_delete/ripple_trim; GUI+MCP share it | protected-surface honesty | — |
