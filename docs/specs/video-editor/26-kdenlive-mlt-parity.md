...
| K-0.4 | Real directional `Wipe`/`Push` passes | 🟠 in progress | [K-B7](#k-b7--luma-map-wipes) | P6 |
| K-0.5 | Thread a `lut_provider` into `graph::compile` so `Lut3d` resolves | 🟠 in progress | [07](07-color-grading.md) acceptance | P7 |
...
| K-0.9 | Make `sync_lock` propagate ripple/insert across sync-locked tracks, **or** remove the control | 🟠 in progress (core half) | protected-surface honesty | — |
