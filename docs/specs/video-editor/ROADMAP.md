...
| **K-B5** compare-effect split | ✅ done | *(this commit)* | Dual compile: full + `skip_clip_looks` (clip effects/grade omitted; upstream nodes share content hash). `EngineCmd::SetCompareEffects` + `EngineFrame.compare_clean`. GUI vertical wipe (A\|B toolbar, draggable divider, `video.compare_effects`). View state only |
...
**Not yet started (next bands):** K-A1 preview chunks / K-A8 subclips / K-A10 fixed playhead; K-B6 param expressions, K-B11 keyframe interchange; K-C2 TagId registry, K-C1 clip-jobs, K-C4 generators, K-C5 archiving, K-C7 import triage; K-D3 per-stream, K-D4 stems; K-E1 YUV scopes + audio spectrum; K-F7 and render polish; K-G1–G4; X-1/X-2/X-3 interop; legal-or-fixture-blocked G/D items.
