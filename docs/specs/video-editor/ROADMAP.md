...
| **K-C2** TagId registry | ✅ done | *(this commit)* | `TagId` + `MediaTag` project registry; `MediaAsset.tag_ids`; `set_asset_tags_resolved` upserts names→ids; pool Tags menu + MCP `set_asset_tags` |
...
**Not yet started (next bands):** K-A1 preview chunks; K-C1 clip-jobs, K-C4 generators, K-C5 archiving, K-C7 import triage; K-D3 per-stream, K-D4 stems; K-E1 YUV scopes + audio spectrum; K-F7 and render polish; K-G1–G4; X-1/X-2/X-3 interop; legal-or-fixture-blocked G/D items. Band-5 await-acceptance gates apply to K-A1/C1/C4/C5/G1–G4/X-*.
