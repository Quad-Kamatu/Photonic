...
| **K-B17** alpha view + unpremultiply | ✅ done | *(this commit)* | Catalogue already had `util.alpha_view` + `util.unpremultiply` (CPU raster-bridge + GPU filter passes). This closes the **monitor view mode** half: `PresentChannel::{Color,Alpha}` on `VideoPresenter`, program-monitor **α** toggle, command `video.alpha_view`. View state (zero undo units). Effects remain the per-clip path for keying polish |
...
**Not yet started (next bands):** K-A1 preview chunks / K-A3 spacer / K-A8 subclips / K-A10 fixed playhead; K-B3 effect zones, K-B5 compare view, K-B6 param expressions, K-B11 keyframe interchange; K-C2 TagId registry, K-C1 clip-jobs, K-C4 generators, K-C5 archiving, K-C7 import triage; K-D3 per-stream, K-D4 stems; K-E1 YUV scopes + audio spectrum; K-F7 and render polish; K-G1–G4; X-1/X-2/X-3 interop; legal-or-fixture-blocked G/D items.
...
| K-B | partial | **K-B1/K-B2 stacks, K-B4 presets, K-B12 easing, K-B14 freeze, K-B15 paste attributes, K-B16 catalogue, K-B17 alpha view done.** Open: K-B3 effect zones, K-B5 compare view, K-B6 param expressions, K-B11 keyframe interchange; manifest `Applicability` gating still open because all 45 entries declare `CLIP_ONLY` | [30](30-effect-catalogue.md), [26 §10](26-kdenlive-mlt-parity.md#10-k-b--effects-and-compositing) |
