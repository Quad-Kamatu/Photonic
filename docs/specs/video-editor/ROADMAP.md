...
| **K-B6** param expressions + reset | ✅ done | *(this commit)* | `param_expr` evaluator (`+−*/()`, `%name` / bare vars, range **refuse** not clamp); middle-click → seed default; wired clip-inspector effect floats + transform rows + color CDL DragValues |
...
**Not yet started (next bands):** K-A1 preview chunks; K-B11 keyframe interchange; K-C2 TagId registry, K-C1 clip-jobs, K-C4 generators, K-C5 archiving, K-C7 import triage; K-D3 per-stream, K-D4 stems; K-E1 YUV scopes + audio spectrum; K-F7 and render polish; K-G1–G4; X-1/X-2/X-3 interop; legal-or-fixture-blocked G/D items.
...
| K-B | partial | **K-B1–B5 stacks/zones/presets/compare, K-B6 expressions, K-B12 easing, K-B14 freeze, K-B15 paste, K-B16 catalogue, K-B17 alpha done.** Open: K-B11 keyframe interchange; manifest `Applicability` gating still open | [30](30-effect-catalogue.md), [26 §10](26-kdenlive-mlt-parity.md#10-k-b--effects-and-compositing) |
