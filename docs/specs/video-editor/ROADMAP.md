...
**Not yet started (gated only):** K-A1/A5, K-C1/C4/C5, K-G1–G4, X-1/X-2/X-3, K-B8/B9 *(Band-5 await acceptance or patent)*; K-B10/K-D2 *(product-blocked)*; K-D1/D-*/G-20 *(legal-or-fixture-blocked)*; **K-F7** one-eval-many-outputs *(engine residual — needs multi-encode design, not a silent open)*; **K-B7 IR binding** *(substrate done; full wipe IR is engine residual)*; **K-F polish** sleep-inhibit / burn-in / add-to-bin / 2-pass *(platform/export polish residual, not Band-5 but out of K-D/E/C critical path)*; **GPU Rec.601 vectorscope twin** *(CPU matrix + UI switch done)*.
