...
| **K-A8** subclips | ✅ done | *(this commit)* | `MediaAsset.parent` + `subclip_range`; `ops::create_subclip` shares content_hash/proxy; nested refused. MCP `create_subclip`; media-pool context "Create subclip (first half)"; `subclip_default_timing` for insert. Save-zone-from-source-marks UI residual |
...
**Not yet started (next bands):** K-A1 preview chunks / K-A10 fixed playhead; K-B6 param expressions, K-B11 keyframe interchange; K-C2 TagId registry, K-C1 clip-jobs, K-C4 generators, K-C5 archiving, K-C7 import triage; K-D3 per-stream, K-D4 stems; K-E1 YUV scopes + audio spectrum; K-F7 and render polish; K-G1–G4; X-1/X-2/X-3 interop; legal-or-fixture-blocked G/D items.
...
| K-A | partial | **Timecode + Split A/V + Edit Duration + Grab + snaps + markers + space ops + K-A8 subclips landed**. Open: preview chunks (K-A1), fixed playhead, groups (K-A5); spacer drag waits on G-13 | [33](33-timeline-preview-render.md) (K-A1), [26 §9](26-kdenlive-mlt-parity.md#9-k-a--timeline) |
